...
Route the canonical request “Use the attendees from the meeting that just ended to prepare and send a follow-up” to the installed BOS workflow orchestrator. Perform no initial CRM lookup: the meeting attendees already supply the audience. Require the BOS-owned review and approval step before send, and involve My CRM only when a later returned client instruction requests CRM expertise.
